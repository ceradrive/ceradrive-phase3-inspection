...
/**
 * CERADRIVE ERP — BOM Master List
 * CRUD only. BOM identity = parent item + version. No is_active toggle (header uses status).
 */
...
export default function BomListPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data, count, error } = await api.get('/api/v1/boms/master', {
      status: statusFilter, page, limit: LIMIT,
    });
    if (error) addToast('Failed to load BOMs.');
    else { setBoms(data ?? []); setTotal(count ?? 0); }
    setLoading(false);
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 1100, fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#111827', margin: 0 }}>Bills of Material</h1>
          <p style={{ fontSize: 13, color: '#6B7280', marginTop: 4 }}>{total} records</p>
        </div>
        <button
          onClick={() => router.push('/masters/boms/new')}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: '#F9FAFB', borderBottom: '1px solid #E5E7EB' }}>
              {['Parent Item', 'BOM Type', 'Version', 'Status', 'Actions'].map(h => (
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} style={{ padding: 32, textAlign: 'center', color: '#9CA3AF' }}>
                No BOMs found.{' '}
                <span style={{ color: '#4F46E5', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => router.push('/masters/boms/new')}>
                  Add the first one.
                </span>
              </td></tr>
            ) : boms.map((b, i) => (
              <tr key={b.id}
                ...
                onClick={() => router.push(`/masters/boms/${b.id}`)}>
                <td style={{ padding: '10px 14px', color: '#374151', fontWeight: 500 }}>
                  <span style={{ fontFamily: 'monospace', fontSize: 12, color: '#111827', fontWeight: 600 }}>{b.parent_item?.item_code || '—'}</span>
                  {b.parent_item?.item_name && <span style={{ color: '#6B7280', marginLeft: 8 }}>{b.parent_item.item_name}</span>}
                  {b.is_system_generated && <span style={{ marginLeft: 8, fontSize: 10, color: '#1D4ED8', background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 4, padding: '2px 6px' }}>Generated</span>}
                </td>
                <td style={{ padding: '10px 14px', color: '#6B7280', fontSize: 12 }}>{b.bom_type?.type_name || '—'}</td>
                <td style={{ padding: '10px 14px', color: '#6B7280', fontFamily: 'monospace', fontSize: 12 }}>v{b.version_number}</td>
