...
const wrap   = { padding: 24, maxWidth: 1040, margin: '0 auto' };
...
function flagSummary(row) {
  const on = FLAG_LABELS.filter(([k]) => row[k]).map(([, label]) => label);
  return on.length ? on.join(', ') : '—';
}

export default function ItemTypeListPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error, meta } = await api.get(`/api/v1/item-types/master?${params.toString()}`);
    if (error) { toast('Failed to load item types.'); setLoading(false); return; }
    setRows(data ?? []);
  ...
  async function toggle(row) {
    const { error } = await api.post(`/api/v1/item-types/master/${row.id}/toggle-active`, { is_active: !row.is_active });
    if (error) { toast('Failed to update status.'); return; }
    toast(`Item type ${!row.is_active ? 'activated' : 'deactivated'}.`);
    load();
  }
  ...
  return (
    <div style={wrap}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
        <h1 style={h1}>Item Type Master</h1>
        <Link href="/masters/item-types/new" style={btn}>+ New Item Type</Link>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {loading
              ...
              : rows.length === 0
                ? <tr><td style={td} colSpan={5}>No item types found.</td></tr>
                : rows.map(r => (
                    <tr key={r.id}>
                      <td style={{ ...td, fontFamily: 'monospace', fontWeight: 600 }}>{r.type_code}</td>
                      <td style={td}>{r.type_name}</td>
                      <td style={{ ...td, color: '#6B7280', fontSize: 13 }}>{flagSummary(r)}</td>
                      <td style={td}><ActiveBadge active={r.is_active} /></td>
                      <td style={{ ...td, textAlign: 'right' }}>
                        <Link href={`/masters/item-types/${r.id}`} style={{ color: '#4F46E5', marginRight: 12, textDecoration: 'none' }}>Edit</Link>
