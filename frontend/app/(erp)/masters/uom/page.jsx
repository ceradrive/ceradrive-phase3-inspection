...
export default function UomListPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error, meta } = await api.get(`/api/v1/uoms/master?${params.toString()}`);
    if (error) { toast('Failed to load UOMs.'); setLoading(false); return; }
    setRows(data ?? []);
  ...
  async function toggle(row) {
    const { error } = await api.post(`/api/v1/uoms/master/${row.id}/toggle-active`, { is_active: !row.is_active });
    if (error) { toast('Failed to update status.'); return; }
    toast(`UOM ${!row.is_active ? 'activated' : 'deactivated'}.`);
    load();
  }
  ...
  return (
    <div style={wrap}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
        <h1 style={h1}>UOM Master</h1>
        <Link href="/masters/uom/new" style={btn}>+ New UOM</Link>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {loading
              ...
              : rows.length === 0
                ? <tr><td style={td} colSpan={5}>No UOMs found.</td></tr>
                : rows.map(r => (
                    <tr key={r.id}>
                      <td style={{ ...td, fontFamily: 'monospace', fontWeight: 600 }}>{r.uom_code}</td>
                      <td style={td}>{r.uom_name}</td>
                      <td style={td}>{r.decimal_places}</td>
                      <td style={td}><ActiveBadge active={r.is_active} /></td>
                      <td style={{ ...td, textAlign: 'right' }}>
                        <Link href={`/masters/uom/${r.id}`} style={{ color: '#4F46E5', marginRight: 12, textDecoration: 'none' }}>Edit</Link>
