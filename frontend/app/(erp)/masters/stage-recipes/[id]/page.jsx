...
function deriveRecipeType(makePolicy, stepList, itemMap) {
  // DP-B1: derived on edit load. Heuristic; user can switch the selector.
  if (String(makePolicy || '').toUpperCase() === 'MAKE_TO_ORDER') return 'MTO_PACKING_SET';
  const outs = (stepList || []).map((s) => itemMap.get(s.output_item_id)).filter(Boolean);
  const mixLike = outs.some((it) => {
    const st = String(it.stage_type || '').toUpperCase();
    const pu = String(it.planning_unit || '').toUpperCase();
    const uom = String(it.uom_code || '').toUpperCase();
    return st === 'MIX' || pu === 'KG' || uom === 'KG';
  });
  if ((stepList || []).length <= 1 && mixLike) return 'MIX_FORMULA';
  return 'MTS_PROCESS_CHAIN';
}
...
export default function StageRecipeEditPage() {
  ...
  const [recipeType, setRecipeType] = useState('MTS_PROCESS_CHAIN');
  ...
  const [copySkuSubmitting, setCopySkuSubmitting] = useState(false);
  const [copySkuTargetItemId, setCopySkuTargetItemId] = useState('');
  const [copySkuStepMap, setCopySkuStepMap] = useState({});
  const [copySkuInputMap, setCopySkuInputMap] = useState({});
  ...
  useEffect(() => {
    async function load() {
      const [itemsRes, processRes, machineRes, uomRes, recipeRes] = await Promise.all([
        api.get('/api/v1/stage-recipes/items', { limit: 500 }),
        api.get('/api/v1/stage-recipes/process-types'),
        api.get('/api/v1/stage-recipes/machines'),
        api.get('/api/v1/boms/uoms'),
        api.get(`/api/v1/stage-recipes/master/${id}`),
      ]);
      setItems(itemsRes.data ?? []);
      setProcesses(processRes.data ?? []);
      setMachines(machineRes.data ?? []);
      setUoms(uomRes.data ?? []);
      if (recipeRes.error || !recipeRes.data) { addToast('Stage recipe not found.'); setLoading(false); return; }
      const d = recipeRes.data;
      setRecipeCode(d.recipe_code ?? '');
      setRecipeName(d.recipe_name ?? '');
      setFgItemId(d.fg_item_id ?? '');
      setPlanningUnit(d.planning_unit ?? 'PCS');
      setMakePolicy(d.make_policy ?? 'MAKE_TO_STOCK');
      setStatus(d.status ?? 'draft');
      setNotes(d.notes ?? '');
      setVersions(d.versions ?? []);
      const loadedSteps = (d.steps ?? []).map(st => ({
        key: st.id || Date.now() + Math.random(),
        step_no: st.step_no,
        output_item_id: st.output_item_id ?? '',
        output_basis_qty: st.output_basis_qty ?? '1',
        output_basis_uom_id: st.output_basis_uom_id ?? st.output_basis_uom?.id ?? '',
        process_type_id: st.process_type_id ?? '',
        machine_id: st.machine_id ?? '',
        calculation_basis: st.calculation_basis ?? '',
        qc_required: !!st.qc_required,
        fpa_required: !!st.fpa_required,
        dependency_step_no: st.dependency_step_no ?? '',
        notes: st.notes ?? '',
        inputs: (st.inputs?.length ? st.inputs : [{ input_item_id: '', qty: 1, uom_id: '', qty_basis: 'PER_BATCH', notes: '' }]).map(inp => ({
          key: inp.id || Date.now() + Math.random(),
          input_item_id: inp.input_item_id ?? '',
          qty: inp.qty ?? '1',
          uom_id: inp.uom_id ?? inp.uom?.id ?? '',
          qty_basis: inp.qty_basis ?? 'PER_BATCH',
          notes: inp.notes ?? '',
        })),
      }));
      setSteps(loadedSteps.length ? loadedSteps : [emptyStep(1)]);
      const localItemById = new Map((itemsRes.data ?? []).map((i) => [i.id, i]));
      setRecipeType(deriveRecipeType(d.make_policy, loadedSteps, localItemById));
      setLoading(false);
    }
    if (id) load();
  }, [id, addToast]);
  ...
  const copySkuStepSourceIds = useMemo(() => [...new Set(steps.map(s => s.output_item_id).filter(Boolean))], [steps]);
  const copySkuInputSourceIds = useMemo(() => [...new Set(steps.flatMap(s => (s.inputs || []).map(i => i.input_item_id)).filter(Boolean))], [steps]);
  ...
  const existingDraftVersion = useMemo(() => {
    return (versions || [])
      .filter((v) => v.status === 'draft' && v.id !== id)
  ...
  const currentVersionNo = useMemo(() => {
    ...
    return Number(current?.version_number || 1);
  }, [versions, id]);

  const nextVersionNo = useMemo(() => {
    const maxV = Math.max(currentVersionNo, ...(versions || []).map((v) => Number(v.version_number || 0)));
    return maxV + 1;
  ...
  async function save() {
    if (status === 'active') { addToast('Active recipe cannot be edited. Create a new version first.'); return; }
    if (status === 'superseded') { addToast('Superseded recipe is locked.'); return; }
    const e = validate();
    if (Object.keys(e).length) { setErrors(e); return; }
    setSaving(true);
    const { data, error } = await api.patch(`/api/v1/stage-recipes/master/${id}`, payload());
    setSaving(false);
    if (error) { addToast(error.message ?? 'Failed to update recipe.'); return; }
    addToast('Stage recipe updated.');
    router.push(`/masters/stage-recipes/${data.id}`);
  }

  async function createNewVersion() {
    if (existingDraftVersion?.id) {
      addToast(`Opening existing draft: ${existingDraftVersion.recipe_code}`);
      router.push(`/masters/stage-recipes/${existingDraftVersion.id}`);
      return;
    }
    setVersioning(true);
    const { data, error } = await api.post(`/api/v1/stage-recipes/master/${id}/new-version`, {});
    setVersioning(false);
    if (error) { addToast(error.message ?? 'Failed to create new version.'); return; }
    addToast(`New draft version created: ${data.recipe_code}`);
    router.push(`/masters/stage-recipes/${data.id}`);
  }

  async function activateVersion() {
    setVersioning(true);
    const { data, error } = await api.post(`/api/v1/stage-recipes/master/${id}/activate`, {});
    setVersioning(false);
    if (error) { addToast(error.message ?? 'Failed to activate version.'); return; }
    addToast(`Activated recipe version: ${data.recipe_code}`);
    router.push(`/masters/stage-recipes/${data.id}`);
  }

  function openCopySkuPanel() {
    setCopySkuOpen(true);
    setCopySkuTargetItemId('');
    setCopySkuStepMap(Object.fromEntries(copySkuStepSourceIds.map((x) => [x, ''])));
    setCopySkuInputMap(Object.fromEntries(copySkuInputSourceIds.map((x) => [x, x])));
  }

  function pickCopySkuTarget(item) {
    setCopySkuTargetItemId(item.id);
    setCopySkuStepMap(Object.fromEntries(copySkuStepSourceIds.map((x) => [x, item.id])));
  }

  async function copyToSku() {
    if (!copySkuTargetItemId) { addToast('Select target output SKU first.'); return; }

    const missingStep = copySkuStepSourceIds.filter((x) => !copySkuStepMap[x]);
    if (missingStep.length) { addToast('Map every step output before copying.'); return; }

    const missingInput = copySkuInputSourceIds.filter((x) => !copySkuInputMap[x]);
    if (missingInput.length) { addToast('Map every input before copying.'); return; }

    setCopySkuSubmitting(true);
    try {
      const { data, error } = await api.post(`/api/v1/stage-recipes/master/${id}/copy-to-sku`, {
        outputItemId: copySkuTargetItemId,
        stepOutputMap: copySkuStepMap,
        inputMap: copySkuInputMap,
      });

      if (error) {
        addToast(error.message ?? 'Failed to copy recipe to SKU.');
        return;
      }

      addToast(data?.recipe_code ? `Copied to SKU draft: ${data.recipe_code}` : 'Copied to SKU draft.');
      if (data?.id) router.push(`/masters/stage-recipes/${data.id}`);
    } finally {
      setCopySkuSubmitting(false);
    }
  }
  ...
  function makeBatchRow() {
    return {
      ...
      stepMap: Object.fromEntries(copySkuStepSourceIds.map((x) => [x, ''])),
      inputMap: Object.fromEntries(copySkuInputSourceIds.map((x) => [x, x])),
    };
  }
  function openBatchPanel() {
    setBatchResults(null);
    setBatchRows([makeBatchRow()]);
    setBatchOpen(true);
  }
  function addBatchRow() {
    setBatchRows((prev) => [...prev, makeBatchRow()]);
  }
  function removeBatchRow(key) {
    setBatchRows((prev) => (prev.length <= 1 ? prev : prev.filter((r) => r.key !== key)));
  }
  function setBatchStep(key, sourceId, itemId) {
    setBatchRows((prev) => prev.map((r) => (r.key === key ? { ...r, stepMap: { ...r.stepMap, [sourceId]: itemId } } : r)));
  }
  function setBatchInput(key, sourceId, itemId) {
    setBatchRows((prev) => prev.map((r) => (r.key === key ? { ...r, inputMap: { ...r.inputMap, [sourceId]: itemId } } : r)));
  }
  function batchRowFilled(r) {
    return copySkuStepSourceIds.every((x) => r.stepMap[x]) && copySkuInputSourceIds.every((x) => r.inputMap[x]);
  }
  function batchRowEmpty(r) {
    return copySkuStepSourceIds.every((x) => !r.stepMap[x]) && copySkuInputSourceIds.every((x) => !r.inputMap[x]);
  }
  async function batchGenerate() {
    const rows = batchRows.filter((r) => !batchRowEmpty(r));
    if (!rows.length) { addToast('Add at least one SKU row with item mappings.'); return; }
    if (rows.some((r) => !batchRowFilled(r))) { addToast('Map every step output and input in each SKU row (or clear the row).'); return; }
    if (rows.some((r) => !r.stepMap[fgItemId])) { addToast('Set the OUTPUT SKU (final item) in each row.'); return; }
    const mappings = rows.map((r) => ({
      outputItemId: r.stepMap[fgItemId],
      stepOutputMap: r.stepMap,
      inputMap: r.inputMap,
    }));
    setBatchSubmitting(true);
    try {
      const { data, error } = await api.post(`/api/v1/stage-recipes/master/${id}/batch-copy-to-sku`, { mappings });
      if (error) { addToast(error.message ?? 'Failed to generate SKU drafts.'); return; }
      setBatchResults(data ?? { results: [], created: 0, failed: 0 });
      addToast(`Created ${data?.created ?? 0} draft(s)${data?.failed ? `, ${data.failed} failed` : ''}.`);
    } finally {
  ...
  return (
    <div style={{ padding: '14px 18px 80px', maxWidth: 1320, fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      ...
      {/* RB-TOP-COMPACT-1 */}
      <div style={{ position: 'sticky', top: 0, zIndex: 20, display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, background: 'rgba(255,255,255,0.97)', border: `1px solid ${TH.border}`, borderRadius: 10, padding: '8px 10px', boxShadow: '0 4px 12px rgba(15,23,42,0.06)', backdropFilter: 'blur(8px)' }}>
        <div style={{ flex: 1 }}>
          <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>Edit Manufacturing Recipe</h1>
          <div style={{ marginTop: 4, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontSize: 11, fontWeight: 800, color: TH.text, background: TH.bg, border: `1px solid ${TH.border}`, borderRadius: 999, padding: '4px 8px' }}>{recipeCode || 'Recipe code'}</span>
            <span style={{ fontSize: 11, fontWeight: 800, color: status === 'active' ? '#15803D' : status === 'draft' ? TH.primary : TH.muted, background: '#fff', border: `1px solid ${TH.border}`, borderRadius: 999, padding: '4px 8px' }}>{status === 'active' ? 'Active (in use)' : status === 'draft' ? 'Draft' : 'Superseded'}</span>
          ...
          {(status === 'active' || status === 'superseded') && (
            <div style={{ marginTop: 4, color: status === 'active' ? TH.primary : TH.muted, fontSize: 12, fontWeight: 600 }}>
              {status === 'active'
                ? existingDraftVersion
                  ? `Active recipe is locked. Draft ${existingDraftVersion.recipe_code} already exists.`
                  : `Active V${currentVersionNo} is locked. Create V${nextVersionNo} before making changes.`
                : 'Superseded recipe is locked for audit history.'}
            </div>
          )}
        </div>
        <button type="button" onClick={openCopySkuPanel} style={{ height: 32, padding: '0 12px', border: `1px solid ${TH.border}`, borderRadius: 6, background: '#fff', color: TH.primary, fontWeight: 800, cursor: 'pointer' }}>Copy to SKU</button>
        <button type="button" onClick={openBatchPanel} style={{ height: 32, padding: '0 12px', border: `1px solid ${TH.border}`, borderRadius: 6, background: '#fff', color: TH.primary, fontWeight: 800, cursor: 'pointer' }}>Generate SKUs from Template</button>
        {status === 'active' && <button onClick={createNewVersion} disabled={versioning} style={{ height: 32, padding: '0 14px', border: 'none', borderRadius: 6, background: TH.primary, color: '#fff', fontWeight: 700, cursor: versioning ? 'default' : 'pointer', opacity: versioning ? 0.65 : 1 }}>{versioning ? 'Opening…' : existingDraftVersion ? `Open Draft V${existingDraftVersion.version_number}` : `Create V${nextVersionNo}`}</button>}
        {status === 'draft' && <button onClick={activateVersion} disabled={versioning} style={{ height: 32, padding: '0 14px', border: `1px solid ${TH.primary}`, borderRadius: 6, background: '#fff', color: TH.primary, fontWeight: 700, cursor: versioning ? 'default' : 'pointer', opacity: versioning ? 0.65 : 1 }}>{versioning ? 'Activating…' : 'Activate Version'}</button>}
      </div>
      ...
      {copySkuOpen && (
        <div style={{ marginBottom: 16, background: '#fff', border: `1px solid ${TH.border}`, borderRadius: 12, padding: 16, boxShadow: '0 1px 3px rgba(15,23,42,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 14 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 15, fontWeight: 900, color: TH.text }}>Copy to SKU</div>
              <div style={{ fontSize: 12, color: TH.muted, marginTop: 3 }}>Creates a new Draft. Source recipe and BOM stay unchanged.</div>
            </div>
            <button type="button" onClick={() => setCopySkuOpen(false)} style={{ height: 32, padding: '0 10px', border: `1px solid ${TH.border}`, borderRadius: 6, background: '#fff', color: TH.muted, cursor: 'pointer', fontWeight: 700 }}>Close</button>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '180px minmax(260px,1fr)', gap: 10, alignItems: 'center', marginBottom: 14 }}>
            <div style={{ fontSize: 12, fontWeight: 900, color: TH.text }}>Target output SKU *</div>
            <ItemSearchSelect
              items={items}
              valueId={copySkuTargetItemId}
              valueLabel={copySkuTargetItemId ? optLabel(itemById.get(copySkuTargetItemId)) : ''}
              placeholder="Search target output SKU…"
              onPick={pickCopySkuTarget}
            />
          </div>

          <div style={{ borderTop: `1px solid ${TH.border}`, paddingTop: 12, display: 'grid', gap: 12 }}>
            <div>
              <div style={{ fontSize: 12, fontWeight: 900, color: TH.text, marginBottom: 8 }}>Step output mapping</div>
              {copySkuStepSourceIds.map((sourceId) => (
                <div key={`step-map-${sourceId}`} style={{ display: 'grid', gridTemplateColumns: 'minmax(220px,1fr) 28px minmax(260px,1fr)', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                  <div style={{ height: 36, display: 'flex', alignItems: 'center', border: `1px solid ${TH.border}`, borderRadius: 6, padding: '0 10px', background: TH.bg, fontSize: 13, fontWeight: 700, color: TH.text }}>{optLabel(itemById.get(sourceId)) || sourceId}</div>
                  <div style={{ textAlign: 'center', color: TH.muted, fontWeight: 900 }}>→</div>
                  <ItemSearchSelect
                    items={items}
                    valueId={copySkuStepMap[sourceId] || ''}
                    valueLabel={copySkuStepMap[sourceId] ? optLabel(itemById.get(copySkuStepMap[sourceId])) : ''}
                    placeholder="Map target step output…"
                    onPick={(it) => setCopySkuStepMap((prev) => ({ ...prev, [sourceId]: it.id }))}
                  />
                </div>
              ))}
            </div>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 900, color: TH.text, marginBottom: 8 }}>Input mapping</div>
              {copySkuInputSourceIds.map((sourceId) => (
                <div key={`input-map-${sourceId}`} style={{ display: 'grid', gridTemplateColumns: 'minmax(220px,1fr) 28px minmax(260px,1fr)', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                  <div style={{ height: 36, display: 'flex', alignItems: 'center', border: `1px solid ${TH.border}`, borderRadius: 6, padding: '0 10px', background: TH.bg, fontSize: 13, fontWeight: 700, color: TH.text }}>{optLabel(itemById.get(sourceId)) || sourceId}</div>
                  <div style={{ textAlign: 'center', color: TH.muted, fontWeight: 900 }}>→</div>
                  <ItemSearchSelect
                    items={items}
                    valueId={copySkuInputMap[sourceId] || ''}
                    valueLabel={copySkuInputMap[sourceId] ? optLabel(itemById.get(copySkuInputMap[sourceId])) : ''}
                    placeholder="Map target input…"
                    onPick={(it) => setCopySkuInputMap((prev) => ({ ...prev, [sourceId]: it.id }))}
                  />
                </div>
              ))}
            </div>
          ...
          <div style={{ marginTop: 14, display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            <button type="button" onClick={() => setCopySkuOpen(false)} style={{ height: 36, padding: '0 14px', border: `1px solid ${TH.border}`, borderRadius: 6, background: '#fff', color: TH.text, cursor: 'pointer', fontWeight: 700 }}>Cancel</button>
            <button type="button" onClick={copyToSku} disabled={copySkuSubmitting} style={{ height: 36, padding: '0 16px', border: 'none', borderRadius: 6, background: TH.primary, color: '#fff', cursor: copySkuSubmitting ? 'default' : 'pointer', opacity: copySkuSubmitting ? 0.65 : 1, fontWeight: 800 }}>{copySkuSubmitting ? 'Copying…' : 'Create Draft Copy'}</button>
          </div>
        </div>
      )}
      ...
      {batchOpen && (
        <div style={{ marginBottom: 16, background: '#fff', border: `1px solid ${TH.border}`, borderRadius: 12, padding: 16, boxShadow: '0 1px 3px rgba(15,23,42,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 14 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 15, fontWeight: 900, color: TH.text }}>Generate SKUs from Template</div>
              <div style={{ fontSize: 12, color: TH.muted, marginTop: 3 }}>One Draft per SKU row. Same process; only item mapping changes. Source recipe and BOM stay unchanged. BOM is generated on Activate only.</div>
            </div>
            <button type="button" onClick={() => setBatchOpen(false)} style={{ height: 32, padding: '0 10px', border: `1px solid ${TH.border}`, borderRadius: 6, background: '#fff', color: TH.muted, cursor: 'pointer', fontWeight: 700 }}>Close</button>
          </div>

          <div style={{ overflowX: 'auto', border: `1px solid ${TH.border}`, borderRadius: 8 }}>
            <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13 }}>
              <thead>
                <tr style={{ background: TH.bg }}>
                  <th style={{ textAlign: 'left', padding: '8px 10px', fontWeight: 900, color: TH.text, borderBottom: `1px solid ${TH.border}`, whiteSpace: 'nowrap' }}>SKU</th>
                  {copySkuStepSourceIds.map((sourceId) => (
                    <th key={`h-step-${sourceId}`} style={{ textAlign: 'left', padding: '8px 10px', fontWeight: 900, color: TH.text, borderBottom: `1px solid ${TH.border}`, minWidth: 240, whiteSpace: 'nowrap' }}>
                      {optLabel(itemById.get(sourceId)) || sourceId}
                      <span style={{ marginLeft: 6, fontSize: 10, fontWeight: 900, color: sourceId === fgItemId ? '#15803D' : TH.muted, background: sourceId === fgItemId ? '#DCFCE7' : '#fff', border: `1px solid ${TH.border}`, borderRadius: 999, padding: '2px 6px' }}>{sourceId === fgItemId ? 'OUTPUT' : 'step output'}</span>
                    </th>
                  ))}
                  {copySkuInputSourceIds.map((sourceId) => (
                    <th key={`h-input-${sourceId}`} style={{ textAlign: 'left', padding: '8px 10px', fontWeight: 900, color: TH.text, borderBottom: `1px solid ${TH.border}`, minWidth: 240, whiteSpace: 'nowrap' }}>
                      {optLabel(itemById.get(sourceId)) || sourceId}
                      <span style={{ marginLeft: 6, fontSize: 10, fontWeight: 900, color: TH.muted, background: '#fff', border: `1px solid ${TH.border}`, borderRadius: 999, padding: '2px 6px' }}>input</span>
                    </th>
                  ))}
                  <th style={{ padding: '8px 10px', borderBottom: `1px solid ${TH.border}` }}></th>
                </tr>
              </thead>
              <tbody>
                {batchRows.map((row, idx) => (
                  <tr key={row.key}>
                    <td style={{ padding: '8px 10px', fontWeight: 800, color: TH.muted, borderBottom: `1px solid ${TH.border}`, verticalAlign: 'top' }}>{idx + 1}</td>
                    {copySkuStepSourceIds.map((sourceId) => (
                      <td key={`c-step-${row.key}-${sourceId}`} style={{ padding: '6px 8px', borderBottom: `1px solid ${TH.border}`, verticalAlign: 'top' }}>
                        <ItemSearchSelect
                          items={items}
                          valueId={row.stepMap[sourceId] || ''}
                          valueLabel={row.stepMap[sourceId] ? optLabel(itemById.get(row.stepMap[sourceId])) : ''}
                          placeholder={sourceId === fgItemId ? 'Target OUTPUT SKU…' : 'Target step output…'}
                          onPick={(it) => setBatchStep(row.key, sourceId, it.id)}
                        />
                      </td>
                    ))}
                    {copySkuInputSourceIds.map((sourceId) => (
                      <td key={`c-input-${row.key}-${sourceId}`} style={{ padding: '6px 8px', borderBottom: `1px solid ${TH.border}`, verticalAlign: 'top' }}>
                        <ItemSearchSelect
                          items={items}
                          valueId={row.inputMap[sourceId] || ''}
                          valueLabel={row.inputMap[sourceId] ? optLabel(itemById.get(row.inputMap[sourceId])) : ''}
                          placeholder="Target input…"
                          onPick={(it) => setBatchInput(row.key, sourceId, it.id)}
                        />
                      </td>
                    ))}
                    <td style={{ padding: '6px 8px', borderBottom: `1px solid ${TH.border}`, verticalAlign: 'top', textAlign: 'center' }}>
                      <button type="button" onClick={() => removeBatchRow(row.key)} disabled={batchRows.length <= 1} style={{ height: 32, padding: '0 10px', border: `1px solid ${TH.border}`, borderRadius: 6, background: '#fff', color: batchRows.length <= 1 ? '#CBD5E1' : '#B91C1C', cursor: batchRows.length <= 1 ? 'default' : 'pointer', fontWeight: 800 }}>✕</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div style={{ marginTop: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
            <button type="button" onClick={addBatchRow} style={{ height: 36, padding: '0 14px', border: `1px solid ${TH.border}`, borderRadius: 6, background: '#fff', color: TH.primary, cursor: 'pointer', fontWeight: 800 }}>+ Add SKU row</button>
            <div style={{ display: 'flex', gap: 10 }}>
              <button type="button" onClick={() => setBatchOpen(false)} style={{ height: 36, padding: '0 14px', border: `1px solid ${TH.border}`, borderRadius: 6, background: '#fff', color: TH.text, cursor: 'pointer', fontWeight: 700 }}>Cancel</button>
              <button type="button" onClick={batchGenerate} disabled={batchSubmitting} style={{ height: 36, padding: '0 16px', border: 'none', borderRadius: 6, background: TH.primary, color: '#fff', cursor: batchSubmitting ? 'default' : 'pointer', opacity: batchSubmitting ? 0.65 : 1, fontWeight: 800 }}>{batchSubmitting ? 'Generating…' : 'Generate Drafts'}</button>
            </div>
          </div>

          {batchResults && (
            <div style={{ marginTop: 14, borderTop: `1px solid ${TH.border}`, paddingTop: 12 }}>
              <div style={{ fontSize: 12, fontWeight: 900, color: TH.text, marginBottom: 8 }}>Results — {batchResults.created ?? 0} created{batchResults.failed ? `, ${batchResults.failed} failed` : ''}</div>
              <div style={{ display: 'grid', gap: 6 }}>
                {(batchResults.results || []).map((r, i) => (
                  <div key={`res-${i}`} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 13 }}>
                    <span style={{ fontWeight: 900, color: r.ok ? '#15803D' : '#B91C1C' }}>{r.ok ? '✓' : '✗'}</span>
                    {r.ok ? (
                      <button type="button" onClick={() => r.recipeId && router.push(`/masters/stage-recipes/${r.recipeId}`)} style={{ border: 'none', background: 'none', color: TH.primary, cursor: 'pointer', fontWeight: 800, padding: 0, textDecoration: 'underline' }}>{r.recipeCode || r.recipeId}</button>
                    ) : (
                      <span style={{ color: TH.muted }}>{optLabel(itemById.get(r.outputItemId)) || r.outputItemId || 'SKU'} — {r.error?.message || 'Failed'}</span>
                    )}
                  </div>
                ))}
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '150px minmax(220px,1fr) minmax(220px,1fr) 140px 160px 120px', gap: 8, marginBottom: 10, alignItems: 'end' }}>
        ...
        <div><label style={lbl}>Status</label><select value={status} disabled style={{ ...ctrl, background: '#F9FAFB' }}><option value="draft">Draft</option><option value="active">Active</option><option value="inactive">Inactive</option><option value="superseded">Superseded</option></select></div>
      </div>
      {showParent && (
        ...
        {steps.map((s) => (
          <div key={s.key} style={{ padding: 0 }}>
            <div style={{ display: 'none' }}>
              <div style={{ fontSize: 13, fontWeight: 800, color: TH.text }}>Step {s.step_no}</div>
              <div style={{ fontSize: 11, fontWeight: 900, color: TH.primary, background: '#EFF6FF', border: `1px solid ${TH.border}`, borderRadius: 999, padding: '5px 10px', letterSpacing: 0.2 }}>
                OUTPUT → PROCESS → BASIS → MATERIALS
              </div>
            </div>
            <div style={{ border: `1px solid ${TH.border}`, borderRadius: 10, background: '#fff', overflow: 'visible', boxShadow: '0 1px 3px rgba(15,23,42,0.05)' }}>
              <div style={{ minHeight: 30, display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', background: TH.bg, borderBottom: `1px solid ${TH.border}` }}>
                ...
                <div style={{ flex: 1 }} />
                {status === 'draft' && <button type="button" onClick={() => removeStep(s.key)} style={{ border: 'none', background: 'transparent', color: '#DC2626', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>Remove</button>}
              </div>
              ...
              <div style={{ padding: 10, display: 'grid', gap: 8 }}>
                ...
                {s.inputs.map((input, idx) => (
                  <div key={input.key} style={{ display: 'grid', gridTemplateColumns: 'minmax(170px,1fr) 62px 68px 104px 26px 76px', gap: 6, alignItems: 'center' }}>
                    ...
                    {status === 'draft' ? <button type="button" onClick={() => removeInput(s.key, input.key)} style={{ height: 32, border: 'none', background: 'transparent', color: TH.muted, cursor: 'pointer', fontSize: 18 }}>×</button> : <div />}
                    {idx === 0 && status === 'draft' ? <button type="button" onClick={() => addInput(s.key)} style={{ height: 32, border: `1px solid ${TH.border}`, borderRadius: 6, background: '#fff', color: TH.primary, cursor: 'pointer', fontSize: 11, fontWeight: 800 }}>+ Material</button> : <div />}
                  </div>
                ))}
              </div>
            </div>
      ...
      <div style={{ position: 'sticky', bottom: 0, zIndex: 20, marginTop: 18, display: 'flex', gap: 10, alignItems: 'center', background: 'rgba(255,255,255,0.96)', border: `1px solid ${TH.border}`, borderRadius: 12, padding: '12px 14px', boxShadow: '0 -8px 20px rgba(15,23,42,0.08)', backdropFilter: 'blur(8px)' }}>
        {status === 'draft' && <button onClick={addStep} style={{ height: 38, padding: '0 14px', border: '1px solid #D1D5DB', borderRadius: 6, background: '#fff', cursor: 'pointer' }}>+ Add Stage</button>}
        <div style={{ flex: 1 }} />
        ...
        {status === 'draft' && <button onClick={save} disabled={saving} style={{ height: 32, padding: '0 14px', border: 'none', borderRadius: 6, background: '#4F46E5', color: '#fff', fontWeight: 600, cursor: saving ? 'default' : 'pointer', opacity: saving ? 0.65 : 1 }}>{saving ? 'Saving…' : 'Save Changes'}</button>}
      </div>
    </div>
  );
}
