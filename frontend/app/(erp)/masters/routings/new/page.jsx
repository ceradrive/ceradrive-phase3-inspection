...
/**
 * CERADRIVE ERP — Create Process Flow (Routing draft)
 * Mandatory: item, routing type (active only), and per step: step name + process type (active only).
 * Input/output item optional. New routing starts as draft. No lifecycle/version here.
 * Steps use a card layout (not a wide table) for mobile-first factory use — the step schema is too
 * wide for a single row; all other patterns mirror the BOM screens exactly.
 */
...
/* Searchable item picker — queries /api/v1/routings/items as you type */
...
export default function RoutingNewPage() {
  ...
  function validate() {
    ...
    if (filled.length === 0) errs.steps = 'Add at least one step.';
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const payloadSteps = steps.filter(isFilled).map((s, i) => ({
      seq_no:           i + 1,
    ...
    const { data, error } = await api.post('/api/v1/routings/master', {
      item_id:         itemId,
    ...
    if (error) {
      ...
      else addToast(error.message ?? 'Failed to create process flow.');
    } else {
      addToast('Process flow created (draft).');
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 920, fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <h1 style={{ fontSize: 20, fontWeight: 700, color: '#111827', margin: 0 }}>New Process Flow</h1>
        <p style={{ fontSize: 12, color: '#9CA3AF', marginTop: 4 }}>Saved as a draft. Item, routing type and at least one step are required.</p>
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, padding: 24, marginBottom: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 18 }}>
          <div>
            ...
            <ItemSearchSelect
              valueId={itemId} valueLabel={itemLabel}
              placeholder="Select item…" error={errors.item}
            ...
            <div key={s.key} style={{ border: '1px solid #E5E7EB', borderRadius: 8, padding: 16, background: '#FCFCFD' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                <div>
                  ...
                  <input value={s.step_name} onChange={e => setStep(s.key, { step_name: e.target.value })} placeholder="e.g. Moulding" style={inputStyle()} />
                </div>
                <div>
                  <label style={labelStyle}>Process *</label>
                  <select value={s.process_type_id} onChange={e => setStep(s.key, { process_type_id: e.target.value })}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                <div>
                  ...
                  <ItemSearchSelect valueId={s.input_item_id} valueLabel={s.input_label} placeholder="Optional…"
                ...
                <div>
                  ...
                  <ItemSearchSelect valueId={s.output_item_id} valueLabel={s.output_label} placeholder="Optional…"
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
                {STEP_FLAGS.map(f => (
                  <label key={f.key} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '7px 10px', border: '1px solid #E5E7EB', borderRadius: 6, fontSize: 13, color: '#374151', cursor: 'pointer', background: s[f.key] ? '#EEF2FF' : '#fff' }}>
                    <input type="checkbox" checked={s[f.key]} onChange={e => setStep(s.key, { [f.key]: e.target.checked })} style={{ width: 18, height: 18, cursor: 'pointer' }} />
                    {f.label}
                  </label>
                ))}
              </div>
              ...
              <div>
                <label style={labelStyle}>Notes</label>
                <input value={s.notes} onChange={e => setStep(s.key, { notes: e.target.value })} placeholder="Optional" style={inputStyle()} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
        ...
        <button onClick={handleSave} disabled={saving}
          ...
          {saving ? 'Saving…' : 'Create Process Flow'}
        </button>
      </div>
